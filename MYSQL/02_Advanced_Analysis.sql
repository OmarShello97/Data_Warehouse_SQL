-- ============================================================
-- ADVANCED BUSINESS ANALYSIS (MySQL version)
-- ============================================================
...
SELECT
    ...
    ROUND(category_sales / NULLIF(SUM(category_sales) OVER (), 0) * 100, 2) AS sales_percentage,
...
SELECT
    ...
    ROUND(subcategory_sales / NULLIF(SUM(subcategory_sales) OVER (), 0) * 100, 2) AS sales_percentage,
    ROUND(subcategory_sales / NULLIF(SUM(subcategory_sales) OVER (PARTITION BY category), 0) * 100, 2) AS category_percentage
...
WITH customer_age_groups AS (
    SELECT
        f.customer_key,
        CASE
            WHEN TIMESTAMPDIFF(YEAR, c.birthdate, CURDATE()) < 20 THEN 'Under 20'
            WHEN TIMESTAMPDIFF(YEAR, c.birthdate, CURDATE()) BETWEEN 20 AND 29 THEN '20-29'
            WHEN TIMESTAMPDIFF(YEAR, c.birthdate, CURDATE()) BETWEEN 30 AND 39 THEN '30-39'
            WHEN TIMESTAMPDIFF(YEAR, c.birthdate, CURDATE()) BETWEEN 40 AND 49 THEN '40-49'
            ELSE '50+'
        END AS age_group,
        SUM(f.sales_amount) AS total_sales,
    ...
    GROUP BY f.customer_key, TIMESTAMPDIFF(YEAR, c.birthdate, CURDATE())
)
SELECT
    age_group,
    ...
    ROUND(SUM(total_sales) / NULLIF(SUM(SUM(total_sales)) OVER (), 0) * 100, 2) AS sales_percentage
...
SELECT
    ...
    ROUND(COUNT(product_key) / NULLIF(SUM(COUNT(product_key)) OVER (), 0) * 100, 2) AS product_percentage
...
SELECT 
    ...
    ROUND(COUNT(customer_key) / NULLIF(SUM(COUNT(customer_key)) OVER (), 0) * 100, 2) AS customer_percentage
